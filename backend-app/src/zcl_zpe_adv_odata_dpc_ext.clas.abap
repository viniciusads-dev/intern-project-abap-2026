...
METHOD /iwbep/if_mgw_appl_srv_runtime~execute_action.
  IF iv_action_name = 'ExecuteProductionProcess'.
    ...
    " Validação de preenchimento básica
    ...
    " Mapeia os valores para as variáveis locais
    lv_material   = ls_param_mat-value.
    lv_quantidade = ls_param_qtd-value.

    " Conversão de zeros à esquerda (ex: transforma '24' em '0024')
    ...
    " 💡 NOVA TRAVA DE SEGURANÇA: Busca o tipo do material no banco de dados
    ...
    IF sy-subrc <> 0.
      " Se o material nem existe no cadastro
      mo_context->get_message_container( )->add_message_text_only(
        iv_msg_type = 'E'
      ...
      elseif lv_tipocm <> '2'. " 🚨 Se o tipo for 1 (Matéria-Prima) ou qualquer outro diferente de 2
      mo_context->get_message_container( )->add_message_text_only(
        iv_msg_type = 'E'
    ...
    " 2. Instancia a Engine de Produção do projeto (Só chega aqui se for Tipo 2)
    lo_engine = NEW zcl_pe_production_engine( ).

    " 3. Executa a lógica envelopada para capturar mensagens clássicas do Module Pool
    TRY.
        IF lo_engine->execute_production( iv_material   = lv_material
                                          iv_quantidade = lv_quantidade ) = abap_true.

          " === CENÁRIO A: ESTOQUE OK E PRODUTO GERADO ===
          DATA: ls_material_retorno TYPE ztpe_material.

          SELECT SINGLE * FROM ztpe_material
            INTO @ls_material_retorno
            WHERE codigocm = @lv_material.
      ...
      CATCH cx_root INTO DATA(lo_ex).
        " === CENÁRIO B: CAPTURA DE ERRO INTERNO DA CLASSE ===
...
METHOD zstr_estoque_oda_update_entity.
  ...
  method ZTPE_BOMSET_CREATE_ENTITY.
    ...
    " Checa se os códigos são válidos
